	...
	private boolean storeNewParticipant(MongoDBClient mongo){
		try{
			...
			try{
				...
				LOGGER.info("Participant \'@"+getUser_name()+"\' already in the competition, not stored");
				return false;
			}
			LOGGER.info("new Participant \'@"+getUser_name()+"\' stored in db and running for the competition.");
